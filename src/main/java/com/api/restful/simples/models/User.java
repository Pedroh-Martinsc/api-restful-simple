...
public class User {
    ...
    } // Inteface para validação do not null, not empty e size
    ...
    // Anotação estrategio para gerar o id no banco de dados "incremento"
    ...
    // Definindo o nome da coluna, quantidade maxima de letras, não podendo ser nulo e sendo unico,
    //Definindo uma segunda validação para o valor não ser uma String vazia nem nulo no username
    @NotNull(groups = CreateUser.class)
    @NotEmpty(groups = CreateUser.class)
    @Size(groups = CreateUser.class, min = 2, max = 100)
    // Definindo valores maximos e minimos do username em tempo real
    private String username;

    @JsonProperty(access = JsonProperty.Access.WRITE_ONLY) // Somente leitura para não retornar a senha no front
    @Column(name = "password", length = 60, nullable = false)
    // Definindo o nome da coluna, quantidade maxima de letras, não podendo ser nulo
